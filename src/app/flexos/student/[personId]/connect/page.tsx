    ...
    const unsub = subscribeToReceipts(selectedId, (receipts) => {
      ...
      setMessages((prev) => {
        ...
        const next = prev.map((m) => {
          ...
          // tik) görmesin, sadece Gönderildi/Teslim Edildi (bu sayfa zaten SADECE
          // öğrenci görünümü). Bu dinleyici Firestore'dan HAM `lastReadAt`'i doğrudan
          // okur, API'nin `readByAll:false` kilidini (`connect-view.ts::buildMessageViews`)
          // bypass eder — burada AYNI kilidi client tarafında da uyguluyoruz (hiç
          // hesaplamıyoruz bile, `lastReadAt` bu ekranda kullanılmıyor).
          const readByAll = false;
