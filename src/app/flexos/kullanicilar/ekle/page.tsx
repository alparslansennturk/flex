...
/** Kullanıcı Ekle'den seçilebilen roller (eğitmen hariç — Eğitmen Ekle'den gelir) */
const CREATABLE_ROLES: RoleKey[] = ["genel_mudur", "egitim_koordinatoru", "ogrenci_isleri", "satis_temsilcisi", "finans"];

/**
 * Sadeleştirilmiş yetki modülleri.
 * Her modül bir "yetki" — açtığında o modülle ilgili tüm CRUD işlemleri açılır.
 * Arka planda registry capability'lere map'lenir (backend'de).
 */
...
/** Rol bazlı varsayılan aktif modüller */
const ROLE_DEFAULT_PERMS: Record<RoleKey, string[]> = {
  genel_mudur: PERM_MODULES.map((m) => m.key), // hepsi
...
export default function KullaniciEklePage() {
  ...
  // varsayılan yetkiler — tüm seçili rollerin birleşimi
  ...
  const rolesLabel = roles.map((r) => ROLE_META[r].label).join(" + ") || "—";
              ...
              {activeTab === "bilgiler" && (
                <div>
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#6F7B87", fontWeight: 500, marginTop: 4, fontStyle: "italic" }}>
