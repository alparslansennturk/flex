        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
              ...
              <button type="button" onClick={onClose} className="am-icon-btn"
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div>
                  <label htmlFor="ad" style={S.label}>Ad <Req /></label>
                  <input id="ad" value={form.ad} onChange={e => setForm(f => ({ ...f, ad: e.target.value }))} placeholder="Ad" style={S.inp} />
                </div>
                <div>
                  <label htmlFor="soyad" style={S.label}>Soyad <Req /></label>
                  <input id="soyad" value={form.soyad} onChange={e => setForm(f => ({ ...f, soyad: e.target.value }))} placeholder="Soyad" style={S.inp} />
                </div>
              </div>
              {/* Telefon + E-posta — en az biri */}
              <div>
                <label htmlFor="telefon" style={S.label}>İletişim <span style={{ color: "#AEB4C0", fontWeight: 400 }}>(telefon veya e-posta — en az biri)</span></label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <input id="telefon" value={form.telefon}
              ...
              {/* Kanal */}
              <div>
                <label htmlFor="kanal" style={S.label}>Kaynak Kanal</label>
                <div style={{ position: "relative" }}>
                  <select id="kanal" value={form.kanal} onChange={e => setForm(f => ({ ...f, kanal: e.target.value as KanalKey }))} style={{ ...S.inp, paddingRight: 36, appearance: "none" }}>
              ...
              {/* Not */}
              <div>
                <label htmlFor="not" style={S.label}>Not <span style={{ color: "#AEB4C0", fontWeight: 400 }}>(opsiyonel)</span></label>
                <textarea id="not" value={form.not} onChange={e => setForm(f => ({ ...f, not: e.target.value }))} placeholder="İlgilendiği eğitim, ek bilgi…"
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 22, paddingTop: 18, borderTop: "1px solid #EEF0F3" }}>
              <button type="button" onClick={onClose} className="am-cancel-btn"
              ...
              <button type="button" onClick={onSave} disabled={saving} className="am-orange-btn"
