  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap", padding: "15px 22px", borderTop: "1px solid #EEF0F3", background: "#F7F8FA" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
        <button type="button" className="am-pag-btn" onClick={() => onPageChange(Math.max(1, safePage - 1))} disabled={safePage <= 1}
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
          <button type="button" key={p} className="am-pag-btn" onClick={() => onPageChange(p)}
        ...
        <button type="button" className="am-pag-btn" onClick={() => onPageChange(Math.min(totalPages, safePage + 1))} disabled={safePage >= totalPages}
