...
export async function POST(req: NextRequest) {
  try {
    ...
    if (await isRateLimited(`pw-reset:${ip}`, 5, 15 * 60 * 1000))
    ...
    // Admin SDK ile şifre sıfırlama linki oluştur
    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "https://flex-one-iota.vercel.app";
    const resetLink = await adminAuth.generatePasswordResetLink(email, {
      url: `${appUrl}/login`,
    });

    // Activation sayfasına yönlendirmek için oobCode'u çıkar
    const url = new URL(resetLink);
    const oobCode = url.searchParams.get("oobCode") ?? "";
    const activationLink = `${appUrl}/login/activation/?oobCode=${oobCode}`;
...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:Inter,Arial,sans-serif">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:40px 16px">
    ...
    console.error("[password-reset] Hata:", err);
