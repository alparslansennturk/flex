...
import { 
  ...
  ArrowBigDownDash, LibraryBig
} from "lucide-react";

type ViewMode = 'Sınıflarım' | 'Şubem' | 'Global';
...
export default function DashboardPage() {
  ...
  const toggleViewMode = () => {
    ...
    else if (viewMode === 'Şubem') setViewMode('Global');
      ...
      <main className="flex-1 lg:pl-64 flex flex-col min-h-screen w-full">
        <header className="h-20 bg-white border-b border-surface-200 px-6 md:px-10 flex items-center justify-between sticky top-0 z-40 w-full">
          ...
          <div className="flex items-center gap-6 shrink-0">
            <div className="relative text-text-secondary cursor-pointer hover:text-[#3A7BD5]"><Bell size={20} /><span className="absolute -top-1 -right-1 w-4 h-4 bg-[#FF4D4D] text-white text-[9px] flex items-center justify-center rounded-full font-bold border-2 border-white">3</span></div>
            <div className="flex items-center gap-4 border-l border-surface-200 pl-6">
              <div className="text-right hidden md:block"><p className="ui-label-small text-text-primary font-bold">Alparslan Şentürk</p><p className="ui-helper-xs text-text-tertiary">Eğitmen | Arı Bilgi</p></div>
              <div className="w-10 h-10 rounded-full border-2 border-[#FF8D28] p-0.5 shrink-0 cursor-pointer transition-transform hover:scale-105"><img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Alparslan" alt="Avatar" className="rounded-full bg-surface-50" /></div>
            </div>
          </div>
        </header>
        ...
        <div className="p-4 md:p-8 lg:p-10 w-full max-w-[1540px] 2xl:max-w-[1700px] mx-auto space-y-8 flex-1">
          <div className="grid grid-cols-12 gap-6 items-stretch">
            <div className="col-span-12 xl:col-span-8 bg-[#10294C] rounded-[32px] p-8 text-white flex flex-col justify-center relative overflow-hidden group border border-white/5 shadow-lg">
              <div className="relative z-10 flex flex-col items-start w-full">
                <span className="ui-helper-xs bg-white/5 px-4 py-1.5 rounded-full border border-white/10 text-[#FF8D28] font-bold capitalize">Atölye analizi</span>
            ...
            <div className="col-span-12 xl:col-span-4 bg-white rounded-[32px] p-6 md:p-8 border border-surface-200 flex flex-col justify-between shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <h3 className="ui-title-xs flex items-center gap-2 font-bold text-text-primary"><Trophy size={18} className="text-[#FF8D28]" /> Sınıf Ligi</h3>
                <button onClick={toggleViewMode} className="flex items-center gap-1.5 text-[#3A7BD5] hover:opacity-80 transition-opacity bg-surface-50 px-3 py-1 rounded-lg border border-surface-100 cursor-pointer active:scale-95"><span className="text-xs md:text-[12px] font-bold capitalize-first">{viewMode === 'Şubem' ? instructorBranch : viewMode}</span><Repeat size={14} className="shrink-0 text-[#FF8D28]" /></button>
              </div>
              <div className="space-y-4 flex-1"><LeaderRow rank={1} name="Mert Demir" status="Zirvede" branch="Kadıköy" statusType="stable" xp="2.850" avatar="Mert" viewMode={viewMode} /><LeaderRow rank={2} name="Selin Yılmaz" status="Yükselişte" branch="Pendik" statusType="rising" xp="2.720" avatar="Selin" viewMode={viewMode} /><LeaderRow rank={3} name="Caner Aydın" status="Düşüşte" branch="Şirinevler" statusType="falling" xp="2.450" avatar="Caner" viewMode={viewMode} /></div>
          ...
          <section className="space-y-6 pt-4">
            <div className="flex items-center gap-3"><Zap className="text-[#FF8D28]" size={24} /><h3 className="ui-title-sm text-text-primary font-bold capitalize-first leading-none">tasarım parkuru</h3></div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6"><ParkourCard title="Kolaj Bahçesi" desc="Sürrealist kompozisyon teknikleri." tag="Ödev" status="Aktif" deadline="Son 2 Gün" iconGradient="bg-gradient-to-b from-pink-500 to-[#B80E57]" tagStyles="bg-pink-100 text-[#B80E57]" btnText="Ödev Ver" icon={<Palette size={24} />} /><ParkourCard title="Kurumsal Kimlik" desc="Marka kimliği ve logo sistemi." tag="Proje" status="Aktif" deadline="14.02.2026" iconGradient="bg-gradient-to-b from-[#1CB5AE] to-[#0E5D59]" tagStyles="bg-[#E0F2F1] text-[#0E5D59]" btnText="Proje Ver" icon={<Briefcase size={24} />} /><ParkourCard title="Kitap Dünyası" desc="Kitap kapağı ve tasarımı" tag="Ödev" status="Pasif" deadline="----------" iconGradient="bg-gradient-to-b from-[#FF8D28] to-[#D35400]" tagStyles="bg-orange-100 text-[#FF8D28]" btnText="Ödev Ver" disabled icon={<BookOpen size={24} />} /></div>
          </section>

          <section className="space-y-6 pt-4 pb-12">
            <div className="flex items-center gap-3 text-text-tertiary px-2"><LibraryBig size={24} /><h3 className="ui-title-sm font-bold capitalize-first leading-none">ödev kütüphanesi</h3></div>
        ...
        {/* FOOTER - 24PX PADDING & IMG BASED ICONS */}
        <footer className="bg-[#10294C] px-[32px] py-[24px] flex items-center justify-between border-t border-white/5 mt-auto min-h-[74px]">
          ...
          <div className="flex flex-col items-end justify-center">
            <div className="flex items-center gap-[16px] mb-[12px]">
              ...
              <SocialIcon src="/icons/x-logo.svg" />
              <SocialIcon src="/icons/instagram.svg" />
            </div>
            <p className="text-[10px] font-light text-white tracking-wide font-inter leading-none">
...
function SocialIcon({ src }: { src: string }) {
  return (
    <div className="w-[24px] h-[24px] bg-white rounded-[4px] flex items-center justify-center cursor-pointer transition-all hover:scale-110 active:scale-95 shadow-sm overflow-hidden">
      <img src={src} className="w-[18px] h-[18px] object-contain" alt="social" />
    </div>
  );
}
...
function StatBox({ label, value, icon }: { label: string, value: string, icon: any }) {
  return (
    <div className="bg-white/5 border border-white/5 p-6 rounded-[24px] flex flex-col items-center justify-center text-center h-full transition-all hover:bg-white/10 shadow-sm cursor-default">
      <div className="w-12 h-12 bg-white/7 rounded-[8px] flex items-center justify-center text-white/70 mb-4">{icon}</div>
      <span className="ui-label-small capitalize text-white/50 mb-2 font-medium">{label}</span>
      <span className="text-[28px] font-extrabold text-white leading-none tracking-tighter">{value}</span>
    </div>
  );
}

function LeaderRow({ rank, name, status, branch, statusType, xp, avatar, viewMode }: any) {
  return (
    <div className="flex items-center justify-between p-2 -mx-2 rounded-xl hover:bg-surface-50 transition-colors cursor-pointer group">
      <div className="flex items-center gap-4 truncate">
        ...
        <div className="w-11 h-11 rounded-full border border-[#D1D5DB] p-0.5 shrink-0"><img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${avatar}`} alt="Avatar" /></div>
        <div className="truncate flex flex-col">
          <div className="flex items-center gap-2">
            <p className="ui-label-small text-text-primary font-bold truncate">{name}</p>
          ...
          <p className={`ui-helper-xs font-semibold capitalize mt-1 tracking-tighter ${rank === 1 ? 'text-[#FF8D28]' : 'text-text-tertiary'}`}>{viewMode === 'Global' ? branch : status}</p>
        </div>
      </div>
      <div className="text-right shrink-0 ml-2"><span className="ui-label-small font-bold text-text-primary leading-none">{xp}<span className="text-text-tertiary uppercase text-[10px]">XP</span></span></div>
    </div>
  );
}

function ParkourCard({ title, desc, tag, status, deadline, iconGradient, tagStyles, btnText, icon, disabled = false }: any) {
  return (
    <div className="bg-white p-8 rounded-[32px] border border-surface-200 flex flex-col justify-between transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:border-transparent group h-full">
      <div className="flex justify-between items-start mb-8">
        ...
        <span className={`px-4 py-1.5 rounded-full text-[11px] font-bold capitalize ${tagStyles}`}>{tag}</span>
      </div>
      <div className="mb-8">
        <h4 className="ui-title-xs text-text-primary mb-1.5 font-bold leading-tight">{title}</h4>
      ...
      <div className="flex items-center justify-between mb-8 bg-[#F8FAFC] rounded-2xl px-6 py-[10px] border border-surface-100">
        <div className="flex flex-col gap-0.5"><span className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest">Durum</span><span className={`text-[12px] font-bold ${status === 'Aktif' ? 'text-text-success' : 'text-text-tertiary opacity-50'}`}>{status}</span></div>
        <div className="flex flex-col gap-0.5 text-right"><span className="text-[10px] text-text-tertiary font-bold uppercase tracking-widest">Teslim</span><span className={`text-[12px] font-bold flex items-center gap-1.5 justify-end ${disabled ? 'text-text-tertiary opacity-50' : 'text-text-primary'}`}><Clock size={12} /><span>{deadline}</span></span></div>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-text-placeholder italic font-semibold opacity-60">Tasarım Atölyesi</span>
        <button disabled={disabled} className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[13px] font-bold transition-all active:scale-95 shadow-sm cursor-pointer ${disabled ? 'bg-surface-200 text-text-placeholder cursor-not-allowed opacity-80' : 'bg-[#6F74D8] text-white hover:bg-[#5E63C2]'}`}>{btnText}{!disabled && <ChevronRight size={16} />}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-w-[calc((100%-96px)/4.3)] snap-start bg-white p-7 rounded-[28px] border border-surface-100 flex flex-col justify-between h-[210px] transition-all hover:shadow-[0_45px_70px_-20px_rgba(0,0,0,0.12)] hover:border-transparent cursor-pointer group">
      <div className="flex items-start gap-4">
        ...
        <div className="truncate"><h5 className="text-[14px] font-bold text-text-primary mb-1 truncate">{title}</h5><p className="text-[11px] text-text-tertiary line-clamp-2">{desc}</p></div>
      </div>
      <div className="border-t border-neutral-200 my-4"></div>
      <div className="flex items-center justify-between">
        <span className="text-[10px] text-text-placeholder italic font-semibold opacity-60">Tasarım Atölyesi</span>
        <button className="px-5 py-2 bg-neutral-200 text-text-primary rounded-xl text-[11px] font-bold flex items-center gap-2 hover:bg-neutral-700 hover:text-white transition-all active:scale-90">Ekle <PlusCircle size={14} /></button>
      </div>
    </div>
  );
}