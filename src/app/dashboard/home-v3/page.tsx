...
function HomeBannerV3() {
  ...
  useEffect(() => {
    const u1 = onSnapshot(query(collection(db, "groups"),   where("status",   "==", "active")), s => setGroupCount(s.size));
    const u2 = onSnapshot(query(collection(db, "students"), where("status",   "==", "active")), s => setStudentCount(s.size));
    const u3 = onSnapshot(query(collection(db, "tasks"),    where("isActive", "==", true)),     s => setTaskCount(s.size));
    return () => { u1(); u2(); u3(); };
  }, []);
