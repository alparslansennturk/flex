...
export default function Header() {
  ...
  return (
    /* Sticky kaldırıldı, sadece normal border-b ve bg-white kaldı */
    <header className="w-full bg-white border-b border-surface-200 font-inter">
      {/* 2000px Hiza Kutusu (Dokunmadım) */}
      <div className="w-[94%] mx-auto h-20 flex items-center justify-between transition-all duration-500 max-w-[1280px] xl:max-w-[1600px] 2xl:max-w-[2000px]">
        ...
        <div className="truncate pr-4">
          <h1 className="text-[24px] text-[#10294C] font-bold leading-tight">Hoş geldin, Alparslan</h1>
          <p className="text-[15px] text-text-tertiary hidden sm:block font-medium">Bugün yeni bir perspektif keşfetmeye ne dersin?</p>
        </div>
        ...
        <div className="flex items-center shrink-0">
          ...
          <div className="relative ml-6" ref={dropdownRef}>
            <button onClick={() => setIsBranchOpen(!isBranchOpen)} className={`flex items-center justify-between gap-2 px-1.5 py-1.5 rounded-xl transition-all w-[118px] border border-transparent ${isBranchOpen ? 'bg-surface-50 border-surface-200' : 'bg-transparent hover:bg-surface-50'}`}><span className="text-[13px] font-bold text-text-tertiary pl-1">{selectedBranch}</span><ChevronDown size={14} className={`transition-transform duration-300 ${isBranchOpen ? 'rotate-180' : ''}`} /></button>
            {isBranchOpen && (
              <div className="absolute top-full left-0 mt-2 w-52 bg-white border border-surface-200 rounded-2xl shadow-xl overflow-hidden z-50">
                {otherBranches.map((branch) => (<button key={branch} onClick={() => { setSelectedBranch(branch); setIsBranchOpen(false); }} className="w-full text-left px-6 py-4 text-[13px] font-medium text-[#10294C] hover:bg-surface-50 border-b border-surface-50 last:border-0">{branch}</button>))}
              </div>
            )}
          </div>
