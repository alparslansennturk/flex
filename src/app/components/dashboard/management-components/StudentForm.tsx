...
export const StudentForm: React.FC<StudentFormProps> = ({
  ...
  <form key={editingStudent?.id || "new-form"} onSubmit={handleSubmit} className={`relative w-full max-w-5xl bg-white rounded-[32px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] h-fit text-[#10294C] transform-gpu will-change-transform ${localShake ? 'animate-shake' : ''}`}>
        ...
        <div className="flex-1 grid grid-cols-2 gap-x-6 gap-y-5">
          ...
          <div className="space-y-1.5 relative">
            ...
            <div className="relative">
              <select name="gender" value={studentGender} onChange={(e) => setStudentGender(e.target.value)} className={`h-12 w-full border rounded-[12px] px-4 pr-10 outline-none appearance-none cursor-pointer transition-all border-neutral-100 bg-neutral-50 focus:border-orange-500 focus:bg-white font-semibold ${!studentGender ? '!text-neutral-600' : '!text-[#10294C] !font-bold'}`}>
