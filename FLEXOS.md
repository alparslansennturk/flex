...
**Durum:** Yoklama Aşama-1 (UI) BİTTİ. Aşama-2: Tatil günleri + kurs ilerleme altyapısı + Ders İstisnası backend'i BİTTİ. **Aşama-2'den HENÜZ YOK:** aylık (bu-ay-özel) planlanan/yapılan stat kartları + course-progress donut (canlıdaki "detailed" mod büyük kartı — şu an sadece lacivert bar var, ayrı büyük kart/donut yok), öğrenci detay modalı, auto-close cron. Tarayıcıda hiç test edilmedi (bu oturumda login yok) — sıradaki oturumda mutlaka denenmeli. Detaylı not: Claude memory `project_attendance_v2_rules.md`.
