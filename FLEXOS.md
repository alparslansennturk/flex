...
- [x] **GRUBA ATA + HAVUZ GERÇEK VERİ + SINIFLAR GERÇEK LİSTE + LIFECYCLE PERSIST BİTTİ (2026-06-22)** — Uçtan uca zincir **satış → havuz (grupsuz) → Gruba Ata → grupta görünür** tıklanarak çalışıyor.
  ...
  - **MODEL BOŞLUKLARI (hâlâ açık):** (1) **Seans saati** Group.schedule'da saklanmıyor (sadece gün+sessionHours) → seans zamanı kaybolur, eklenmeli; (2) **Grup Sil** hâlâ client-only (DELETE backend yok) → silinen grup reload'da geri gelir.
...
- [ ] **SIRADAKİ** — Grup Sil backend (DELETE /groups/[id]) · Seans saatini Group.schedule'a kaydet · Roster/sınıf detay görünümü (gruba tıkla→öğrenci listesi; eğitmen dummy) · **Şube Aşama-2** (kullanıcıya şube ata → `Actor.branchIds`, liste uçlarını şubeye göre süz) · Ödeme UI (taksit/tahsilat) · Kurumsal **Firmalar** ([[kurumsal-modul-firmalar]]) · Katalogda bireysel/kurumsal **ayrı gruplu liste**.
