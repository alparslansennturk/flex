...
**Durum:** Yoklama Aşama-1 (UI) BİTTİ. Aşama-2: Tatil günleri + kurs ilerleme altyapısı + Ders İstisnası backend'i + Yoklama Raporu (DOĞRU portlanmış hali) + sidebar 3-alt-menü yapısı BİTTİ. **Aşama-2'den HENÜZ YOK (öncelik sırasına göre):** Hakediş hesaplama (Finans, saat×hourlyRate — Rapor sayfasındaki eğitmen özeti üzerine kurulacak), aylık (bu-ay-özel) planlanan/yapılan stat kartları + course-progress donut (canlıdaki "detailed" mod büyük kartı — Yoklama Al/Detay'da, Rapor'da DEĞİL), auto-close cron, Core'da eğitmenin "kendi ders saatim" görünümü (tasarım kararı bekliyor — tek istisna, teknik engel yok). Öğrenci detay modalı düşük öncelik (kullanıcı zaten canlıda yapmıştı). **Tarayıcıda HİÇ test edilmedi** (bu oturumda login yok) — bu oturumda çok fazla birikti, sıradaki oturumda mutlaka uçtan uca denenmeli. Detaylı not: Claude memory `project_attendance_v2_rules.md`.
