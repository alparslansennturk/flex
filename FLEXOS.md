...
- **DURUM DÜZELTME (2026-06-25):** **Ödeme FAZ-2 esasen BİTTİ** — satış formu `payment` (peşin+senet) gönderiyor (`satis-yap` satır 343-369 → `flexos_payments` persist); eski "satış formu payment göndermiyor" notu **GEÇERSİZ**. Kalan tek kozmetik: havuz **LİSTESİNDE** ödeme durumu rozeti (veri `GET /persons`'ta zaten hazır, tabloya basılmadı).
- [ ] **YARIN — ÖNCELİK SIRASI (2026-06-25 onayı):** (1) **Satış İptali** = Satış Listesi'nde, **soft + cascade** — yok: `sale.cancel` capability + `cancelSale` servisi (`Sale.status` active|cancelled + cancelledAt/by/reason + **enrollment→cancelled cascade**, roster aktif-only olduğu için sınıftan otomatik düşer) + `POST /api/flexos/sales/[id]/cancel` + Satış Listesi'ne iptal butonu + sebep modalı + havuzda "İptal" filtresi. Person/Enrollment SİLİNMEZ (audit/remarketing) → [[project-sales-list-cancel]]. SONRA (2) **Kullanıcılar/yetki sıfırdan** (flexos_users + UI + kullanıcı→Actor + rol→paket köprüsünü **gerçek enforcement**'la değiştir; şu an auth-actor herkesi admin pakete bağlıyor) → [[project-sube-scope]]. Sonra (3) Eğitmen v2.
- [ ] **SIRADAKİ** — **Kullanıcılar/yetki katmanı sıfırdan** (flexos_users + UI; canlı öncesi şart — [[project-sube-scope]]) (gruba tıkla→öğrenci listesi; eğitmen dummy; UI yapılınca — acil değil) · **Ödeme FAZ-2** (Payment persist: PaymentRepo + `flexos_payments` + createSale'e ödeme dizisi + assertion) · **Şube Aşama-2** (kullanıcıya şube ata → `Actor.branchIds`, liste uçlarını şubeye göre süz) · **Kullanıcılar/yetki katmanı sıfırdan** (flexos_users + UI; canlıya çıkış öncesi — [[project-sube-scope]]) · Ödeme UI (taksit/tahsilat) · Kurumsal **Firmalar** ([[kurumsal-modul-firmalar]]) · Katalogda bireysel/kurumsal **ayrı gruplu liste** · Canlı trainer rebuild ([[project-trainer-rebuild]]).
