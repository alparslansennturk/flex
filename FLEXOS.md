...
- **Ödev Yönetimi** (`/flexos/odevler/yonetim`) — canlıdaki `TaskManagementPanel.tsx` (1095 satır, global admin tablo: Şablon Yönetimi/Mevcut Ödevler/Arşiv/Ödev Havuzları/Lig sekmeleri) **BİREBİR portlanmadı** — kullanıcı: "ödev yönetimini şimdi bu haliyle ekle, sonra revize ederiz" → sade bir grup-kartı→CRUD ekranı: grup kartları (aynı `GroupCard`) → o grubun ödev listesi + "Yeni Ödev" modalı (başlık/açıklama/son teslim tarihi/durum) + düzenle/sil. FlexOS'un TEK canonical `assignTask`/`updateAssignment`/`deleteAssignment` servisine bağlı.

**Düzeltme geçmişi (aynı gün, ders çıkarılacak):** Önce Ödev Yönetimi'ni Ödev Teslimi'nin neredeyse kopyası olarak yaptım → kullanıcı "aynı sayfa" dedi → bunu "ikisi gerçekten aynı, birleştir" diye yanlış yorumlayıp Yönetimi SİLDİM → kullanıcı düzeltti: "ben sana ödev yönetimi ekle dedim, canlıya bak dedim, sen sildin". Doğrusu: ikisi canlıda da FARKLI ekranlar (Teslimi=grup bazlı takip, Yönetimi=global CRUD tablo) — Yönetimi geri eklendi (basit haliyle, kullanıcı onayladı: "sonra revize ederiz"), Teslimi CRUD'suz salt-okunur haline geri döndü. **Ders:** kullanıcı bir şeyi "aynı" diye eleştirdiğinde bunu otomatik "birleştir/sil" diye okuma — önce hangi sayfanın yanlış/eksik yapıldığını sor.
