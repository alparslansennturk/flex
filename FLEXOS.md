...
- [x] **KAPSAM NETLEŞTİ — yol haritası revize (2026-06-24, kullanıcı kararı; aşağıdaki eski SIRADAKİ önceliklerini EZER):**
  ...
  2. **Faturalandırma YOK** (Logo/muhasebe değiliz) — finans yardımı yalnız: **tahsilat takibi + "ödemesi gelecek/geçen öğrenci" hatırlatması** + **eğitmen aylık hakediş = yoklama × hourlyRate**. **Ödeme alınınca tek tık → öğrenci "beklemede"den "aktif"e döner** (grupsuz/gruplu fark etmez; satışta Beklemede, tahsilatta Aktif — ileride FAZ-2). `Sale.billing` dormant; vergi/fatura makinesi yok → eski 2026-06-19 fatura kararı GERİ ALINDI. ([[project-invoicing-billing]])
  ...
  - **REVİZE ÖNCELİK SIRASI:** (1) Ödeme FAZ-2 (tahsilat + hatırlatma + beklemede→aktif) → (2) Satış Listesi + iptal(cascade) + rapor → (3) Kullanıcı/rol/yetki → (4) Eğitmen v2.
  - **⚠️ AÇIK — kullanıcı sonraki oturumda anlatacak:** "Beklemede" statüsünde bir DEĞİŞİKLİK yapılacak (detay henüz verilmedi, token bitti). Yeni oturumda önce bunu sor/dinle; beklemede→aktif kuralı buna göre revize olabilir.
