...
- [ ] **SIRADAKİ — "FlexOS'a devam" DENİNCE DOĞRUDAN BU (kullanıcı 2026-06-18 PRE-ONAY verdi: sormadan başla, bu Eğitim bölümünü TAMAMEN bitir): Faz-2 Düzenle (içerik yapısı).** Düzenleme modunda bölüm/track ekle-çıkar ŞU AN KAYDEDİLMİYOR (sadece eğitim scalar alanları). Gereken: section/track UPDATE+DELETE uçları + servis + kaydederken "sil-yeniden-oluştur" + bölüm/track reverse-map persist. Sonra kullanıcının DİKEY DİLİMİ: öğrenci satış UI → grup ekle UI (eğitmen havuzu = `users` instructor'lardan) → demo satış → grup-eğitmen-eğitmende görünür. Kurumsal **Firmalar** modülü ileride ([[kurumsal-modul-firmalar]]). Katalogda bireysel/kurumsal **ayrı gruplu liste** (asla iç içe).
