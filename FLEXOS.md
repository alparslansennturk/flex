...
- [ ] **SIRADAKİ — iş "B": `createSale` + kayıt** — `eduos/sale.ts`/`payment.ts` şu an sadece TİP (servis/route YOK). Yazılacak: `createSale` servisi + `POST /api/flexos/sales` → Person(öğrenci) + Enrollment(track scope) oluştur; Sale'e **guardianName/guardianIdNo** (18 altı) + fatura tarafı alanları. Ödeme/sözleşme havuzu sonra (elde gerçek sözleşme yok). Sonra: grup ekle UI (eğitmen havuzu=`users` instructor) → demo satış → grup-eğitmen-eğitmende görünür. Kurumsal **Firmalar** ileride ([[kurumsal-modul-firmalar]]). Katalogda bireysel/kurumsal **ayrı gruplu liste**.
