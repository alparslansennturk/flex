...
- [ ] **flexos_users hesap bağlama (authUid) YOK** — "Kullanıcı Ekle" sadece Firestore doc yazıyor, gerçek Firebase Auth hesabı/aktivasyon akışı kurulmamış (canlıdaki `/api/users/create`+kod gibi). Gerçek yetki bağlamanın (aşağıdaki madde) ön koşulu.
- [ ] **Gerçek yetki bağlama (RoleDef.permModules → `can()`)** — henüz sadece veri/görsel katman, gerçek capability kontrolüne etkisi yok. İki ön koşulu var: yukarıdaki hesap bağlama + `auth-actor.ts::actorFromCaller`'ın async'e geçmesi (50+ senkron call site, riskli refactor). Kullanıcı bunu yapınca fake test hesaplarıyla deneyecek.
- [ ] **Satış Yap: hibrit eğitim "Teslim Şekli" (Yüz Yüze/Online)** → **BİTTİ (2026-07-08)** — Full Paket'te `deliveryOptions`'a göre ayrı fiyat seçilebiliyor artık, Track Bazlı hariç (kapsam dışı).
